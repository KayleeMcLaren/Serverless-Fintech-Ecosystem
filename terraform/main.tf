...
resource "aws_api_gateway_deployment" "api_deployment" {
  ...
  # We now combine the outputs from ALL THREE modules.
  triggers = {
    redeployment = sha1(
      "${module.digital_wallet.api_integrations_json}${module.micro_loan.api_integrations_json}${module.payment_processor.api_integrations_json}"
    )
  }
  # ---------------------------------
